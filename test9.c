...
typedef struct data_hist
{
    short Static;       //过去的状态
    short Speed;        //过去的速度值

    struct data_hist *next;

}Data_Hist;

//创建循环单链表，记录过去的变量值
Data_Hist* InitList(short num)
{
    Data_Hist* head = (Data_Hist*)malloc(sizeof(Data_Hist));
    head->Static = 0;
    head->Speed = 0;
    ...
    for(i;i<num-1;i++)
    {
        Data_Hist* node = (Data_Hist*)malloc(sizeof(Data_Hist));
        node->Speed = 0;
...
void traction_control()
{
    ...
    //Encoder.Tar = (Adc.Hori_Err < 0.2 && Adc.Hori_Err > -0.2)?280:270;
    Encoder.Tar = 300;
...
void ramp_exit_control()
{
    ...
    if(speed > 5000){delay = ramp_exit_delay_defa;}
    else {speed = 0;}
    if(delay > 0)   {delay--;}
    else            {state = 0;}
}
...
{
    dir_val = 260 +  180 * Adc.Hori_Err;//240
    if(delay > 0)   {delay--;}
    else            {state = 0;}
}
